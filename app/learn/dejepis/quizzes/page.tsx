        ...
        (async () => {
            try {
                ...
                if (!json?.questions || !Array.isArray(json.questions)) {
                    throw new Error("Neplatný formát: čekám { questions: [...] }");
                }
    ...
    const score = useMemo(() => {
        ...
        for (const qq of data.questions) {
            ...
            if (qq.type === "mcq") {
                ...
                if (typeof v === "string") {
                    // jednoduché porovnání: trim + lowercase
            ...
            TEST
                ...
                <div className="flex flex-wrap items-end justify-between gap-3">
                    <h1 className="text-3xl font-semibold tracking-tight sm:text-5xl">
                        {data?.title ?? "Quizzes"} <span className="text-neutral-400">mode</span>
                    </h1>
                ...
                <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div className="lg:col-span-2 rounded-2xl border border-black/10 bg-white/70 p-6 shadow-sm backdrop-blur">
                        {loading ? (
                            <div className="text-sm text-neutral-600">Načítám otázky…</div>
                        ) : error ? (
                            <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">
                                {error}
                            </div>
                        ) : !q ? (
                            <div className="text-sm text-neutral-600">Žádné otázky.</div>
                        ) : (
                            <>
                    ...
                    <div className="rounded-2xl border border-black/10 bg-white/70 p-6 shadow-sm backdrop-blur">
                            ...
                            {submitted && score ? (
                                <div className="mt-2 text-sm text-neutral-700">
                                    ...
                                    <div className="mt-2 text-xs text-neutral-500">
                                        U short answer se porovnává přesně (bez diakritiky to zatím neřeším).
                                    </div>
                                </div>
                            ) : (
                        ...
                        <div className="mt-4 text-xs text-neutral-500">
                            Backend si napojíš na svůj generátor. Frontend čeká `/api/quizzes`.
                        </div>
                    </div>
                </div>
            </div>
